...
async def handle_command(command, args, message, bot_client):
    ...
    # (to prevent unnecesary spam if the bot shares the command prefix with some other)
    ...
    if cmd_obj.params and len(args) < len(cmd_obj.params):
        await bot_client.send_message(message.channel, message.author.mention + " Insufficient parameters!")
    else:
